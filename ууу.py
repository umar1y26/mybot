...
# ============= КОНФИГ =============
BOT_TOKEN = "8990574965:AAG7ImooLRDyEie_n9uXq7t5j6WLolJvDjY"
...
CARD_COOLDOWN = 1 * 60 * 60
PREMIUM_CARD_COOLDOWN = 2 * 60 * 60
BONUS_COOLDOWN = 3 * 60 * 60
DICE_COOLDOWN = 5 * 60
PREMIUM_DICE_COOLDOWN = 60
...
def user_mention(u: dict) -> str:
    """username если есть, иначе кликабельная ссылка на имя через tg://user?id=..."""
...
async def get_or_create_user(tg_user) -> dict:
    ...
    if user:
        # Обновляем username/имя если изменилось
        upd = {}
...
class AdminGive(StatesGroup):
    target = State()      # ждём user_id
    amount = State()      # ждём количество
    card_pick = State()   # ждём выбор карты (через callback)
...
async def cmd_profile(msg: Message):
    ...
    text = (
        f"<b>Профиль «{escape(user['nickname'])}»{prem_mark}</b>\n\n"
        f"<blockquote>"
        ...
        f"{'<br>🚀 <b>PREMIUM</b> до ' + time.strftime('%d.%m.%Y', time.localtime(user['premium_until'])) if is_premium(user) else ''}"
        f"</blockquote>"
    )
    ...
    if fav_photo:
        try:
            await msg.answer_photo(fav_photo, caption=text, reply_markup=kb); return
        except Exception:
            pass
...
async def cb_inventory(cq: CallbackQuery):
    ...
    await cq.message.answer(text); await cq.answer()
...
async def cb_mycards(cq: CallbackQuery):
    ...
    if not items:
        await cq.message.answer("У вас пока нет карт. Напишите <code>andrusho</code>"); await cq.answer(); return
    for it in items[:10]:
        try:
            ...
            if card.get("photo_id"):
                await cq.message.answer_photo(card["photo_id"], caption=cap)
            else:
                await cq.message.answer(cap)
        except Exception:
            continue
    await cq.answer()
...
async def try_give_card(msg: Message):
    ...
    if elapsed < cd:
        left = cd - elapsed
        await msg.answer(f"Вы посмотрели, но <b>Andrusho</b> не было рядом 🙈\n\n⏳ <b>{fmt_time_left(left)}</b>")
        return
    ...
    if not card:
        await msg.answer("😿 Карт в базе нет"); return
    ...
    try:
        if card.get("photo_id"):
            await msg.answer_photo(card["photo_id"], caption=caption)
        else:
...
async def check_subs(user_id: int) -> list:
    """Если бот не админ канала → get_chat_member падает → доверяем юзеру (НЕ добавляем в not_subbed)."""
    not_subbed = []
    for ch in CHANNELS:
        try:
...
async def cmd_bonus(msg: Message):
    ...
    if elapsed < BONUS_COOLDOWN:
        ...
        await msg.answer(f"🎁 Следующий бонус через <b>{fmt_time_left(left)}</b>"); return
    ...
    await msg.answer(f"🎉 Получено <b>{amount}</b> бонусный сундук!", reply_markup=kb)
...
async def cb_top(cq: CallbackQuery):
    ...
    try:
        ...
        await cq.message.answer("❌ Ошибка получения топа"); await cq.answer(); return
    if not users:
        await cq.message.answer("Топ пуст — никто ещё ничего не набрал"); await cq.answer(); return
...
async def cmd_shop(msg: Message):
    ...
    await msg.answer("🛍 <b>Магазин</b>", reply_markup=kb)
...
async def cb_shop_chests(cq: CallbackQuery):
    ...
    await cq.message.answer(text, reply_markup=kb); await cq.answer()
...
async def cb_shop_items(cq: CallbackQuery):
    ...
    text = (
        ...
        f"⏱ Ускоритель — <b>20</b> монет (сброс кулдауна карты)\n"
        f"🍀 Удача — <b>25</b> монет (повышенная редкость)\n\n"
        f"💰 Баланс: <b>{user['coins']}</b></blockquote>"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⏱ Ускоритель [20💰]", callback_data="buy_item:speed")],
...
async def cmd_premium(msg: Message):
    text = (
        "🚀 <b>Premium — 30 ⭐</b>\n\n<blockquote>"
        "1. ⏳ Карта раз в <b>2 часа</b> (вместо 4ч)\n"
        "2. 🎲 Кости раз в <b>1 минуту</b> (вместо 5мин)\n"
...
async def cb_open_chests(cq: CallbackQuery):
    user = await get_or_create_user(cq.from_user)
    bonus = user.get("bonus_chests", 0)
    epic = user.get("epic_chests", 0)
    legend = user.get("legend_chests", 0)
    text = (
        f"📦 <b>Твои сундуки</b>\n\n<blockquote>"
        f"🎁 Бонусный: <b>{bonus}</b>\n"
        f"💜 Эпический: <b>{epic}</b>\n"
        f"💛 Легендарный: <b>{legend}</b></blockquote>\n\n"
        f"Купить эпич/лег: {DONATE_CHANNEL}"
    )
    rows = []
    if bonus > 0: rows.append([InlineKeyboardButton(text=f"🎁 Бонусный ({bonus})", callback_data="open_chest:bonus")])
    if epic > 0:  rows.append([InlineKeyboardButton(text=f"💜 Эпический ({epic})", callback_data="open_chest:epic")])
    if legend > 0: rows.append([InlineKeyboardButton(text=f"💛 Легендарный ({legend})", callback_data="open_chest:legend")])
    rows.append([InlineKeyboardButton(text="🛒 Купить (⭐)", url=DONATE_CHANNEL)])
    await cq.message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows))
    await cq.answer()
...
async def cb_open_chest(cq: CallbackQuery):
    ...
    field_map = {"bonus": "bonus_chests", "epic": "epic_chests", "legend": "legend_chests"}
    field = field_map[kind]
    user = await get_or_create_user(cq.from_user)
    if user.get(field, 0) < 1:
        await cq.answer("❌ Нет таких сундуков", show_alert=True); return
    if kind == "bonus":
        rarity = random.choices(["common", "rare", "epic"], weights=[60, 30, 10])[0]
    elif kind == "epic":
        rarity = random.choices(["rare", "epic", "mythic"], weights=[40, 50, 10])[0]
    else:
        rarity = random.choices(["epic", "mythic", "legendary"], weights=[30, 40, 30])[0]
    card = await roll_card(rarity)
    if not card:
        await cq.answer("❌ В базе нет карт", show_alert=True); return
    ...
    rd = RARITIES[card["rarity"]]
    caption = (
        f"📦 <b>Из сундука выпало:</b>\n\n<b>«{escape(card['name'])}»</b>\n<blockquote>"
        f"💎 {rd['emoji']} {rd['name']}\n✨ +{rd['points']:,}\n💰 +{rd['coins']}</blockquote>"
    )
    if card.get("photo_id"):
        await cq.message.answer_photo(card["photo_id"], caption=caption)
    else:
        await cq.message.answer(caption)
    await cq.answer()
...
async def cmd_index(msg: Message):
    ...
    await msg.answer(f"📜 <b>Индекс карт</b> ({total})\nВыберите редкость:", reply_markup=kb)
...
async def cb_index(cq: CallbackQuery):
    ...
    if key == "all":
        ...
        if not cards:
            await cq.message.answer("Карт нет"); await cq.answer(); return
        lines = [f"📜 <b>Все карты ({len(cards)})</b>\n"]
        for c in cards:
    ...
    if not cards:
        await cq.message.answer(f"В категории <b>{RARITIES[key]['name']}</b> карт нет"); await cq.answer(); return
    rd = RARITIES[key]
    await cq.message.answer(f"{rd['emoji']} <b>{rd['name']} карты ({len(cards)})</b>")
    for c in cards[:30]:
        cap = f"{rd['emoji']} <b>{escape(c['name'])}</b>\n💎 {rd['name']}"
        if c.get("photo_id"):
            try:
                await cq.message.answer_photo(c["photo_id"], caption=cap)
            except Exception:
                await cq.message.answer(cap)
        else:
            await cq.message.answer(cap)
...
async def cmd_market(msg: Message):
    ...
    await msg.answer("🏪 <b>Маркетплейс</b>\nВыберите редкость:", reply_markup=kb)
...
async def cb_market(cq: CallbackQuery):
    ...
    if not items:
        await cq.message.answer(f"В <b>{RARITIES[rarity]['name']}</b> пусто"); await cq.answer(); return
    rd = RARITIES[rarity]
    for it in items:
        ...
        if it.get("photo_id"):
            try:
                await cq.message.answer_photo(it["photo_id"], caption=cap, reply_markup=kb)
            except Exception:
                await cq.message.answer(cap, reply_markup=kb)
        else:
            await cq.message.answer(cap, reply_markup=kb)
    await cq.answer()
...
async def cb_market_buy(cq: CallbackQuery):
    ...
    await cq.message.answer(f"✅ Куплена <b>{escape(card['name'])}</b>!")
    await cq.answer()
...
async def cmd_roleplay(msg: Message):
    ...
    await msg.answer(f"🎭 <b>РП</b> (в ответ на сообщение):\n<blockquote>{acts}</blockquote>")
...
async def handle_rp(msg: Message, action: str):
    if not msg.reply_to_message or not msg.reply_to_message.from_user:
        await msg.answer("Ответьте на сообщение игрока"); return
    target = msg.reply_to_message.from_user
    if target.id == msg.from_user.id:
        await msg.answer("Себя нельзя 😉"); return
    action = action.strip().lower()
    for key, (emoji, tpl) in RP_ACTIONS.items():
        if action.startswith(key):
            a = escape(msg.from_user.first_name or "Игрок")
            b = escape(target.first_name or "Игрок")
            await msg.answer(f"{emoji} " + tpl.format(a=f"<b>{a}</b>", b=f"<b>{b}</b>"))
            return
    await msg.answer("Неизвестное действие. /roleplay")
...
async def handle_marriage(msg: Message, action: str):
    ...
    if action.startswith("создать брак") or action == "брак":
        if not msg.reply_to_message or not msg.reply_to_message.from_user:
            await msg.answer("Ответьте на сообщение"); return
        target = msg.reply_to_message.from_user
        if target.id == uid or target.is_bot:
            await msg.answer("Нельзя"); return
        ...
        if existing:
            await msg.answer("💔 Кто-то уже в браке"); return
        await proposals_col.insert_one({
            "from_id": uid, "to_id": target.id, "chat_id": msg.chat.id, "created_at": now_ts(),
        })
        await msg.answer(
            f"💍 <b>{escape(msg.from_user.first_name)}</b> → <b>{escape(target.first_name)}</b>\n"
    ...
    if action.startswith("принять"):
        ...
        if not p: await msg.answer("Нет предложений"); return
    ...
    if action.startswith("развод"):
        ...
        if not m: await msg.answer("Вы не в браке"); return
...
async def handle_quote(msg: Message, kind: str, payload: str):
    ...
    if not text:
        await msg.answer(f"<code>andrusho {kind} «текст»</code>"); return
    emoji = "💬" if kind == "цитата" else "🎨"
    await msg.answer(f"{emoji} <blockquote>«{escape(text)}»\n— <i>{escape(author or '')}</i></blockquote>")
...
async def cb_take_admin(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.answer("🔐 Введите пароль администратора:")
...
async def show_admin_panel(msg: Message):
    ...
    await msg.answer("👑 <b>Админ-панель</b>", reply_markup=kb)
...
async def cb_adm_give_menu(cq: CallbackQuery):
    ...
    await cq.message.answer("🎁 <b>Что выдать?</b>", reply_markup=kb); await cq.answer()
...
async def cb_give_choose(cq: CallbackQuery, state: FSMContext):
    ...
    if gtype == "card":
        # Сначала спросим user_id, потом покажем редкости
        await cq.message.answer("👤 Введите <b>user_id</b> игрока:")
...
async def fsm_give_amount(msg: Message, state: FSMContext):
    ...
    gtype = data["give_type"]
    if gtype == "premium":
        new_until = max(now_ts(), (await users_col.find_one({"user_id": uid})).get("premium_until", 0)) + amount * 86400
        ...
        await msg.answer(f"✅ Премиум выдан на <b>{amount}</b> дней до {time.strftime('%d.%m.%Y', time.localtime(new_until))}")
    else:
        await users_col.update_one({"user_id": uid}, {"$inc": {gtype: amount}})
        await log_action(msg.from_user.id, f"give_{gtype}", f"to={uid} +{amount}")
        await msg.answer(f"✅ Выдано <b>{amount}</b> [{gtype}] игроку <b>{escape(data['target_name'])}</b>")
    await state.clear()
...
async def cb_givec_rarity(cq: CallbackQuery, state: FSMContext):
    ...
    if not cards:
        await cq.message.answer("В этой редкости карт нет"); await cq.answer(); return
    kb_rows = []
    for c in cards[:30]:
        kb_rows.append([InlineKeyboardButton(text=f"{RARITIES[rarity]['emoji']} {c['name']}",
                                              callback_data=f"givec_card:{c['_id']}")])
    await cq.message.answer(f"Выберите карту ({RARITIES[rarity]['name']}):",
...
async def cb_adm_users(cq: CallbackQuery):
    ...
    for i in range(0, len(text), 3500):
        await cq.message.answer(text[i:i+3500])
    await cq.answer()
...
async def cb_adm_userinfo(cq: CallbackQuery):
    if not is_admin(cq.from_user.id):
        await cq.answer("❌", show_alert=True); return
    await cq.message.answer("Используйте <code>/userinfo &lt;user_id&gt;</code>"); await cq.answer()
...
async def cb_adm_loadmarket(cq: CallbackQuery):
    if not is_admin(cq.from_user.id):
        await cq.answer("❌", show_alert=True); return
    await cq.message.answer("🏪 <code>/loadmarket &lt;card_id&gt; &lt;цена&gt;</code>\nID карт — /listcards"); await cq.answer()
...
async def cb_adm_logs(cq: CallbackQuery):
    ...
    if not logs:
        await cq.message.answer("📋 Логов нет"); await cq.answer(); return
...
async def cb_adm_broadcast(cq: CallbackQuery):
    if not is_admin(cq.from_user.id):
        await cq.answer("❌", show_alert=True); return
    await cq.message.answer("📢 <code>/broadcast &lt;текст&gt;</code>"); await cq.answer()
...
async def cmd_userinfo(msg: Message):
    ...
    if len(parts) != 2:
        await msg.answer("Формат: /userinfo &lt;user_id&gt;"); return
    try:
        uid = int(parts[1])
...
async def cmd_logs(msg: Message):
    ...
    q = {}
    if len(parts) == 2:
        try: q = {"user_id": int(parts[1])}
        except: await msg.answer("❌"); return
    logs = await logs_col.find(q).sort("ts", -1).limit(50).to_list(length=50)
    if not logs: await msg.answer("Нет логов"); return
    lines = []
    for l in logs:
        t = time.strftime("%d.%m %H:%M", time.localtime(l["ts"]))
        lines.append(f"[{t}] <code>{l['user_id']}</code> · {l['action']} · {escape(str(l.get('details','')))}")
...
async def cmd_broadcast(msg: Message):
    if not is_admin(msg.from_user.id): return
    parts = msg.text.split(maxsplit=1)
    if len(parts) < 2: await msg.answer("Формат: /broadcast текст"); return
    text = parts[1]
...
async def cmd_addcard(msg: Message, state: FSMContext):
    ...
    await msg.answer("Выберите редкость новой карты:", reply_markup=kb)
...
async def cb_add_rar(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.answer(f"Редкость: <b>{RARITIES[rarity]['emoji']} {RARITIES[rarity]['name']}</b>\nНазвание карты:")
...
async def fsm_addname(msg: Message, state: FSMContext):
    if not msg.text: await msg.answer("Текстом"); return
...
async def cmd_delcard(msg: Message):
    if not is_admin(msg.from_user.id): return
    parts = msg.text.split(maxsplit=1)
    if len(parts) < 2: await msg.answer("Формат: /delcard ID"); return
    try: cid = ObjectId(parts[1].strip())
    except: await msg.answer("❌"); return
    c = await cards_col.find_one({"_id": cid})
    if not c: await msg.answer("Не найдена"); return
    await cards_col.delete_one({"_id": cid})
    await inventory_col.delete_many({"card_id": str(cid)})
    await msg.answer(f"🗑 «{escape(c['name'])}»")
...
async def cmd_listcards(msg: Message):
    ...
    if not cards: await msg.answer("Пусто. /addcard"); return
...
async def cmd_loadmarket(msg: Message):
    if not is_admin(msg.from_user.id): return
    parts = msg.text.split()
    if len(parts) != 3: await msg.answer("Формат: /loadmarket card_id цена"); return
    try:
    ...
    await msg.answer(f"✅ <b>{escape(c['name'])}</b> @ {price}💰")
